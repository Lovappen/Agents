#!/bin/bash
# cc-connect-setup.sh — 把 nako 接到 cc-connect 多平台 host
#
# 装/复用 cc-connect，确保 ~/.cc-connect/config.toml 里有指向 openclaw acp 的
# nako project，然后引导用户 QR-onboard 飞书/微信等平台。
#
# Usage:
#   bash scripts/cc-connect-setup.sh [--with-feishu] [--with-weixin] [--non-interactive] [--agent-id <id>]
#
# Flags:
#   --with-feishu       自动跑 feishu QR 引导（若未配 feishu）
#   --with-weixin       自动跑 weixin QR 引导（若未配 weixin）
#   --non-interactive   不询问，缺什么就跳过（不报错）

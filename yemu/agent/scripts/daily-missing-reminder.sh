...
# 奈子的每日思念提醒辅助脚本
#
# 该脚本由 openclaw cron `yemu-missing-reminder` 通过 agentTurn 间接驱动 —
...
MISSING_MSG="${YEMU_DAILY_REMINDER_MSG:-主人大人～现在是下午4:50啦 ❤️ 奈子在战斗女仆的执勤间隙都在想你哦…今天累不累？要不要奈子给你做个甜点等你回来～}"
...
if [ $sent -eq 0 ] && command -v openclaw >/dev/null 2>&1 && [ -n "${YEMU_DAILY_REMINDER_CHAT:-}" ]; then
  # openclaw 原生：必须显式指定 channel + chat_id
  openclaw message send --channel "${YEMU_DAILY_REMINDER_CHANNEL:-feishu}" --target "$YEMU_DAILY_REMINDER_CHAT" "$MISSING_MSG" >/dev/null 2>&1 && sent=1
fi
...
if command -v doki >/dev/null 2>&1; then
  ...
  doki connect "${YEMU_DOKI_DEVICE:-DK-META2}" >/dev/null 2>&1 || true
  doki action vibration "${YEMU_DOKI_VIBRATION:-25}" >/dev/null 2>&1 || true
  sleep 3
